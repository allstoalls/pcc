...
#
# 8192 entries (64 KiB), direct-mapped.  This table memoizes an ADDRESS, so a
# first probe of a fresh object can never hit on its own; allocation churn,
# not capacity, sets that floor.  `_granule_object_slot` has already proved
# the cell at publish time, so `pcc_gc_granule_object_publish` seeds the entry
# there rather than paying the walk on first use.  Capacity was then measured
# on the virtual-thread gateway workload, where the miss path was 5.8% of
# samples once the hit path had been inlined: 256 -> 8192 entries removed
# 1.49% of all process instructions.  65536 entries (512 KiB) removed 2.01%,
# and was DENIED -- the extra instruction saving did not convert and process
# cycles regressed against 8192 (4.008e9 vs 3.959e9) because the table stopped
# being data-cache resident.  An earlier 256 -> 1024 test measured only +0.60%
# QPS while the predicate was still one fused function, where its call and
# frame cost masked the miss.
define_global_i64_array(
...
def pcc_gc_granule_object_publish(ptr) -> i64:
    ...
    if observed == 5783538902897647429 or observed == 5783538902897647428:
        ...
        # Seed the exact positive cache here so a fresh object's first
        # refcount operation takes the inlined hit path instead of the radix
        # walk.  A first probe can never hit on its own: the cache remembers
        # addresses, so allocation churn, not capacity, sets its miss floor.
        # This does not weaken the predicate.  Every hit still acquire-loads
        # the LIVE lifecycle word, so an entry left behind by a retired cell
        # answers -1 rather than a false positive, and the store uses the same
        # address, slot arithmetic and relaxed ordering as the one the miss
        # path already performs after its own validation.
        published_bits: i64 = ptr_diff(ptr, null())
        atomic_store_i64(
            global_addr("pcc_allocator_exact_object_cache"),
            (
                (
                    logical_shift_right_i64(published_bits, 4)
                    ^ logical_shift_right_i64(published_bits, 12)
                )
                & 8191
            )
            * 8,
...
def pcc_gc_granule_is_object_start(ptr) -> i64:
    ...
    chain is therefore fused rather than decomposed, and every check is kept
    in its original order: table acquire-load, nonzero granule key,
    acquire-load probe, object kind, validated carve count, 4 KiB-aligned
    base, slab bounds, exact cell alignment, then the acquire-load of the LIVE
    lifecycle word.  The decomposed helpers remain exported and unchanged for
    their own callers and for the focused granule tests.

    Only the exact-cache hit stays in this function; the descriptor and radix
    validation lives in `_granule_object_start_uncached` so that the hit path
    is small enough for `inline-defined` to place it in its callers.  That
    took the merged runtime IR from 130 calls to this predicate down to 84
    plus 47 outlined miss calls, inlining the probe into 46 further sites, and
    on its own measured a paired +3.65% handler QPS with -5.0% process cycles
    against the retained runtime.  Instructions moved only -0.20%: the win is
    the removed call, frame and root bookkeeping, not arithmetic.

    The outline also created a place where a check costs nothing on the hit
    path, which is what let the publish-time seed and the larger exact cache
    pay; see the `pcc_allocator_exact_object_cache` note.  Together the three
    measured a paired +5.62% handler QPS and -2.08% instructions against the
    retained runtime over seven 200k-request repeats, every repeat positive,
    and +4.01% paired QPS against CPython asyncio on the same runs.  Keep the
    miss path outlined when editing either half.
    """
    if ptr_is_null(ptr) != 0:
        return -1
...
@c_abi_export("pcc_allocator_granule_object_start_uncached")
def _granule_object_start_uncached(ptr, object_bits: i64, object_slot: i64) -> i64:
    # Keep descriptor/radix validation out of the exact-cache hit path.
    object_cache = global_addr("pcc_allocator_exact_object_cache")
    key: i64 = logical_shift_right_i64(ptr_diff(ptr, null()), 12)
    if key <= 0 or logical_shift_right_i64(key, 48) != 0:
        return -1
